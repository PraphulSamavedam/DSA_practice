...
class Solution:
    """
    This class has various solutions for LeetCode question 1052
    'Grumpy Bookstore Owner'
    """

    def brute_approach(self, customers: List[int],
                       grumpy: List[int], minutes: int) -> int:
        """
        Time complexity: O(n * m ) where n = len(customers) = len(grumpy)
        ...
        for idx in range(length):
            if grumpy[idx] == 0:
                default += customers[idx]

        # Now unlock
        for unl in range(length):
            unblocked = 0
            for x in range(unl, unl + minutes):
                if x < length and grumpy[x] == 1:
                    unblocked += customers[x]
            result = max(result, unblocked + default)
        return result

    def optimal_approach(self, customers: List[int],
                         grumpy: List[int], minutes: int) -> int:
        """
    ...
    def testCases(self):
        ...
        for indx, _ in enumerate(customers_inps):
            soln_result = self.brute_approach(customers=customers_inps[indx],
        ...
        for indx, _ in enumerate(customers_inps):
            soln_result = self.optimal_approach(customers=customers_inps[indx],
                                                grumpy=grumpy_inps[indx],
                                                minutes=minutes_inps[indx])
            test_fail.append(soln_result == results[indx])
            soln_results.append(soln_result)
